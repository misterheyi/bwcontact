...
public class JwMenuAPI {
	...
	public static List<WeixinButton> getAllMenu(String accessToken) throws WexinReqException{
		...
		Object error = result.get(WeiXinConstant.RETURN_ERROR_INFO_CODE);
		String menu = JSONObject.parseObject(result.toJSONString()).getString("menu");
		String buttons = JSONObject.parseObject(menu).getString("button");
		List<WeixinButton> lstButton = JSON.parseObject(buttons, new TypeReference<List<WeixinButton>>(){});
		return lstButton;
	}
