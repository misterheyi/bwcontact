...
public class JwOrderManagerAPI {
	...
	public static OrderInfo getByOrderId(String newAccessToken, String order_id) {
		if (newAccessToken != null) {
			...
			orderInfo = JSON.parseObject(info.toJSONString(),  OrderInfo.class);
			return orderInfo;
		}
		return null;
	}
	...
	public static List<OrderInfo> getByFilter(String newAccessToken, OrderPara orderPara) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "GET", JSON.toJSONString(orderPara));
			// 正常返回
	...
	public static OrderRtnInfo doDelivery(String newAccessToken, OrderDelivery orderDelivery) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", JSON.toJSONString(orderDelivery));
			OrderRtnInfo orderRtnInfo = JSON.parseObject(result.toJSONString(), OrderRtnInfo.class);
			return orderRtnInfo;
		}
		return null;
	}
