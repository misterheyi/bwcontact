...
public class JwProductAPI {
	...
	public static CommodityRtnInfo doAddCommodity(String newAccessToken, Product product) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", JSON.toJSONString(product));
			CommodityRtnInfo commodityRtnInfo = JSON.parseObject(result.toJSONString(), CommodityRtnInfo.class);
	...
	public static Product getCommodity(String newAccessToken, String product_id) {
		if (newAccessToken != null) {
			...
			product = JSON.parseObject(info.toJSONString(),  Product.class);
			return product;
		}
		return null;
	}
