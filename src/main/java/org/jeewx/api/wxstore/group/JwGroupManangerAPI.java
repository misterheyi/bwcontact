...
public class JwGroupManangerAPI {
	...
	public static GroupRtnInfo doAddGroupManager(String newAccessToken, Group group) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", JSON.toJSONString(group));
			GroupRtnInfo groupRtnInfo =  JSON.parseObject(result.toJSONString(), GroupRtnInfo.class);
			return groupRtnInfo;
		}
		return null;
	}
	...
	public static GroupRtnInfo doUpdateGroupManagerProduct(String newAccessToken, GroupProductInfo groupProductInfo) {
		if (newAccessToken != null) {
			...
			JSONObject result = WxstoreUtils.httpRequest(requestUrl, "POST", JSON.toJSONString(groupProductInfo));
			GroupRtnInfo groupRtnInfo = JSON.parseObject(result.toJSONString(), GroupRtnInfo.class);
			return groupRtnInfo;
		}
		return null;
	...
	public static GroupDetailInfo getByGroupId(String newAccessToken, Integer group_id) {
		if (newAccessToken != null) {
			...
			groupDetailInfo = JSON.parseObject(info.toJSONString(), GroupDetailInfo.class);
